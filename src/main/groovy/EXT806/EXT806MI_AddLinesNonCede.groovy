/************************************************************************************************************************************************
Extension Name: EXT806MI.AddLinesNonCede
Type: ExtendM3Transaction
Script Author: NRAOEL
Date: 2024-10-22
Description:
* Add Lines to the dynamic table EXT806 (virement, lettrage, impayee, remboursement non cédés)
...
public class AddLinesNonCede extends ExtendM3Transaction {
  ...
  public AddLinesNonCede(MIAPI mi, ProgramAPI program, DatabaseAPI database, MICallerAPI miCaller) {
    this.mi = mi
  ...
  /**
   * @readEcritureBancaire - get lines from FGLEDG   
   * @params
   * @returns
   */
  void readEcritureBancaire() {
    ExpressionFactory expression = database.getExpressionFactory("FGLEDG")
    expression = expression.eq("EGAIT1", compteDedieFactor)
  ...
  /**
   * @getCUGEX3 - List FEID that correspond to an invoice payment
   * @params - 
   * @returns - tx40 : FEID in FGLEDG
   */
  ...
  /**
   * @add103104VirementNonCedee - REG03 : create 103 104 (Virement) , list associated lines
   * @params - tyli, ait1, jrno, vono, yea4, vser, feid, cino, pycl, trcd, jsno, cuam, lineNumber
   * @returns - array
   */
  void add103104VirementNonCedee(int lineNumber) {
    ...
    if (tyli.equals("103") && !ait1.trim().equals(compteCollectifClient) && (pycl.trim().equals("3") || pycl.trim().equals("0")) && dbcr.equals("C")) {
      ...
      String bkac103104 = compteDedieFactor
      ...
      DBAction queryAll = database.table("FGLEDG")
        .index("00")
        .selection("EGVONO", "EGVSER", "EGACDT", "EGCUCD", "EGCUAM", "EGVTXT", "EGAIT1", "EGFEID", "EGFNCN", "EGTRCD", "EGDBCR")
        .matching(expAll)
        .build()
  ...
  /**
   * @add104ImpayeNonCedee - REG03 : create 103 104 (Impayé), , list associated lines
   * @params - tyli, ait1, jrno, vono, yea4, vser, feid, cino, pycl, trcd, jsno, cuam, lineNumber
   * @returns - array
   */
  void add104ImpayeNonCedee(int lineNumber) {
    String tyli = output.get(lineNumber).get("TYLI")
    String ait1 = output.get(lineNumber).get("AIT1")
    int jrno = Integer.parseInt(output.get(lineNumber).get("JRNO"))
    String vono = output.get(lineNumber).get("VONO")
    ...
    String feid = output.get(lineNumber).get("FEID")
    String pycl = output.get(lineNumber).get("PYCL")
    String dbcr = output.get(lineNumber).get("DBCR")
    ...
    if (tyli.equals("103") &&
      ...
      ExpressionFactory expCheck = database.getExpressionFactory("FGLEDG")
      expCheck = expCheck.eq("EGVONO", vono).and(expCheck.eq("EGVSER", vser)).and(expCheck.eq("EGFEID", feid)).and(expCheck.eq("EGAIT1", compteDedieFactor)).and(expCheck.eq("EGDBCR", "C"))
