/************************************************************************************************************************************************
Extension Name: EXT806MI.AddLinesChqNC
Type: ExtendM3Transaction
Script Author: NRAOEL
Date: 2024-10-22
Description:
* Add Lines to the dynamic table EXT806 (Chèque non cédé)
...
public class AddLinesChqNC extends ExtendM3Transaction {
  ...
  public AddLinesChqNC(MIAPI mi, ProgramAPI program, DatabaseAPI database, MICallerAPI miCaller) {
    this.mi = mi
  ...
  public void main() {
    ...
    inCKNO = mi.inData.get("CKNO").isBlank() ? "" : mi.inData.get("CKNO").trim()
    inBKID = mi.inData.get("BKID").isBlank() ? "" : mi.inData.get("BKID").trim()

    exclusionList = getCompteCUGEX()
  ...
  /**
   * @readYEA4JRNOJSNO - get line from FARRED   
   * @params
   * @returns
   */
  void readYEA4JRNOJSNO() {
    String bkid = String.format("%-5s", inBKID)
    rawCkno = bkid + inCKNO

    ExpressionFactory expression = database.getExpressionFactory("FSLEDX")
    expression = expression.eq("ESSEXI", rawCkno)

    DBAction query = database.table("FSLEDX")
      .index("00")
      .selection("ESYEA4", "ESJRNO", "ESJSNO")
      .matching(expression)
      .build()
  ...
  void readFSLEDG() {
    ...
    expression = expression.lt("ESACDT", getDATE().toString()).and(expression.eq("ESTRCD", "10").or(expression.eq("ESTRCD", "20")).or(expression.eq("ESTRCD", "21")))
  ...
  void addCustomFields() {
    for (int i = 0; i < output.size(); i++) {
      if (output.get(i).get("EXST").equals("OK")) {
        ...
        remittance = checkFYRMNB(output.get(i).get("PYNO"), inCKNO, inBKID)
        output.get(i).put("RMNO", remittance)

        output.get(i).put("ACSO", getRAI1(output.get(i).get("PYNO"), inCKNO, inBKID))
        output.get(i).put("BKAC", compteDedieFactor)
        ...
        add103104ChequeNonCede(i)
      }
    }
  }

  /**
   * @getVono - Get Voucher Number 
   * @params - yea4, jrno, jsno
   * @returns - 
   */
  String getVono(int yea4, int jrno, int jsno) {
    ...
    String result = ""
    if (query.read(container)) {
      result = container.get("EGVONO").toString().trim()
    }
    return result
  }
  ...
  /**
   * @addInfoCat - add info for processed line in FGLEDX
   * @params - cono, divi, yea4, jrno, jsno, gexi
   * @returns -
   */
  void addInfoCat() {
    LocalDateTime dateTime = LocalDateTime.now()
    String yea4 = dateTime.format(DateTimeFormatter.ofPattern("yyyy")).toInteger()

    Map < String, String > params = ["CONO": inCONO.toString(), "DIVI": inDIVI, "YEA4": yea4, "JRNO": "1", "JSNO": "1", "GEXI": rawCkno]
  ...
  /**
   * @add103104ChequeNonCede - REG03 : create 103 104 (Effet non cédé)
   * @params - tyli, ait1, jrno, vono, yea4, vser, feid, cino, pycl, trcd, jsno, cuam, lineNumber
   * @returns - array
   */
  void add103104ChequeNonCede(int lineNumber) {
    ...
    String cino = output.get(lineNumber).get("CINO")
    String pycl = output.get(lineNumber).get("PYCL")
    Float cuam = Float.parseFloat(output.get(lineNumber).get("CUAM"))
    String dbcr = output.get(lineNumber).get("DBCR").trim()

    if (tyli.equals("103") && !ait1.equals(compteCollectifClient) && pycl.equals("2") && dbcr.equals("C")) {
      ExpressionFactory expression = database.getExpressionFactory("FGLEDG")
      expression = expression.eq("EGVONO", vono)
        .and(expression.eq("EGVSER", vser))
        .and(expression.eq("EGFEID", feid))
        .and(
          expression.eq("EGTRCD", "10")
          .or(expression.eq("EGTRCD", "20"))
          .or(expression.eq("EGTRCD", "21"))
        )
