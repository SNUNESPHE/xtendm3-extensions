/****************************************************************************************
 Extension Name: EXT090MI.UpdEntitlmtInfo
...
public class UpdEntitlmtInfo extends ExtendM3Transaction {
  ...
  public UpdEntitlmtInfo(MIAPI mi, DatabaseAPI database, LoggerAPI logger, ProgramAPI program, UtilityAPI utility, MICallerAPI miCaller) {
    this.mi = mi
  ...
  public void main() {
    ...
    if (!queryEXT090.readLock(EXT090, updateCallBackEXT090)) {
      mi.error("Droit " + ENNO + " n'existe pas")
      return
    }
  }
  ...
  // Update EXT090
  Closure<?> updateCallBackEXT090 = { LockedResult  lockedResult ->
    LocalDateTime timeOfCreation = LocalDateTime.now()
    int changeNumber = lockedResult.get("EXCHNO")
    if (mi.in.get("QTRC") != null)
      lockedResult.set("EXQTRC", inQTRC)
    if (mi.in.get("QTRA") != null)
      lockedResult.set("EXQTRA", inQTRA)
    if (mi.in.get("QREC") != null)
      lockedResult.set("EXQREC", inQREC)
    if (mi.in.get("QREA") != null)
      lockedResult.set("EXQREA", inQREA)
    if (mi.in.get("QTNC") != null)
      lockedResult.set("EXQTNC", inQTNC)
    if (mi.in.get("QTNA") != null)
      lockedResult.set("EXQTNA", inQTNA)
