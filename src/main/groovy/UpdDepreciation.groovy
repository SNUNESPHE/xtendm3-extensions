/************************************************************************************************************************************************
 Extension Name: EXT010MI.UpdDepreciation
 Type: ExtendM3Transaction
 Script Author: ARNREN
 Date: 2024-09-10
 Description:
 * Update depreciation
 * This extension is used by scheduler
 ...
 ARNREN        2024-09-10  1.0       5190 - Ecran de gestion de la décote
 ARNREN        2024-12-04  1.1       MMS850MI.AddReclass handling added
 ARNREN        2025-11-26  1.2       nbMaxRecord replaced, CUGEX1 readAll limit reduced
 **************************************************************************************************************************************************/
...
public class UpdDepreciation extends ExtendM3Transaction {
  ...
  private Integer NB_MAX_RECORD = 50
  ...
  public void main() {
    ...
    logger.debug("ITNO = " + inITNO)
    logger.debug("RGDT = " + inRGDT)
    logger.debug("RGTM = " + inRGTM)
    logger.debug("TMSX = " + inTMSX)
    ...
    if (queryMITTRA.read(MITTRA)) {
      ...
      logger.debug("lecture de MITTRA ")
      ...
      if (!queryCUGEX1.readAll(CUGEX1, 5, NB_MAX_RECORD, outDataCUGEX1)) {
        return
      }
      ...
      if (!query.readAll(CUGEX11, 5, NB_MAX_RECORD, outDataCUGEX11)) {
        return
      }
