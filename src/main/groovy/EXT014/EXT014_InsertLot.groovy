/****************************************************************************************
 Extension Name: EXT014MI/InsertLot
 ...
 * Add item into the table EXT014. 
 ...
 Tovonirina ANDRIANARIELO   2025-09-05       1.1              Xtend validation
******************************************************************************************/
...
public class InsertLot extends ExtendM3Transaction {
  ...
  public InsertLot(MIAPI mi, ProgramAPI program, DatabaseAPI database) {
    this.mi = mi
  ...
  public void main() {
    ...
    DBAction insertQuery = database.table("EXT014")
      .index("00")
      .build()
    ...
    insertContainer.set("EXFROM", inFROM)
    insertContainer.set("EXITTO", inITTO)
    insertContainer.set("EXNUMB", inNUMB)
    insertContainer.set("EXLTDT", Integer.parseInt(inLTDT))
    insertContainer.set("EXSUCC", inSUCC)
    // audit fields
  ...
  boolean validateSUCC(String status) {
    List<String> validStatus = ["PROCESSING", "SUCCESS", "CANCELLED"]
    if (validStatus.contains(status)) {
      return true
    } else {
      mi.error("Le statut ${status} n'est pas valide. Les valeurs valides sont PROCESSING, SUCCESS, ou CANCELLED")
      return false
    }
  }
