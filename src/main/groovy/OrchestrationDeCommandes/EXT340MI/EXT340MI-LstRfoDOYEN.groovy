...
* 1.0.1      140126  KMOTEAN     Added filter to have unique ROUT / Set default values for MITBAL fields / Adjusted function calculateNextDeliveryDate to take date according to timezone
*/
...
public class LstRfoDOYEN extends ExtendM3Transaction {
  ...
  private boolean isPartiellementDisponible = false

  
  public LstRfoDOYEN(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
  ...
  public void main() {
    ...
    if(!validateCONO()){
      ...
      mi.outData.put('LNTX', "CONO/DIVI inconnue")
      mi.write()
      return
    }
    ...
    if(!validateCUNO()){
      ...
      mi.outData.put('CEAN', inCEAN)
      mi.outData.put('LNST', "I")
      mi.outData.put('LNTX', "Client inconnu")
      mi.write()
      return
    }
    ...
    if(inORQA==0){
      ...
      mi.outData.put('CEAN', inCEAN)
      mi.outData.put('LNST', "I")
      mi.outData.put('LNTX', "Quantité invalide")
      mi.write()
      return
    }
      ...
      if(assortmentCheck=="0"){
        ...
        if(listAssortment.size()>0){
          if(!checkAssortimentArticle()){
            ...
            mi.outData.put('LNST', "N")
            mi.outData.put('LNTX', "Reference inconnue")
            mi.outData.put('AVTX', "Indisponible")
            mi.outData.put('AVST', "I")
            mi.outData.put('AV01', "0.0")
            mi.outData.put('TOMU', "1")
            mi.write()
            return
    ...
    listRoutesAllDetails.each { record1 ->
      ...
      if(!correctITNOFound){
      ...
      else if(record1.AV01=="" || record1.AV01==null){
        ...
        if(retievedTOMU!="" && record1.AV01.toString().trim()!=""){
          ...
          // mi.outData.put('AV01', pbqa.toString())
          record1.AV01=pbqa.toString()
        }

        if(Double.parseDouble(record1.AV01)==0.0){
          record1.AVTX="Indisponible"
          record1.AVST="I"
        }
          ...
          record1.AVST="P"
          isPartiellementDisponible = true
        }
      }
    ...
    // Keep only the first occurrence of each ROUT
    listRoutesAllDetails = listRoutesAllDetails.unique { it.ROUT }
    ...
    if(inOALT==0){
      ...
      else if(isPartiellementDisponible){
        lnst="P"
        lntx="Partiellement disponible"
      }
      else{
        lnst="I"
        lntx="Non disponible"
      ...
      listRoutesAllDetails.each { record1 ->
        ...
        mi.outData.put('MODL', record1.MMDL)
        mi.outData.put('LNST', lnst)
        mi.outData.put('LNTX', lntx)
  ...
  */
  int findMultiple(double remainingQuantity,double maxItemsToAllocate, double TOMU) {
   ...
   /**
  * @retrieveAssortimentOIS071 - Gets ITDS/FUDS/CFI1 from MITMAS
  * @params -
  * @returns -
  */
  void retrieveAssortimentOIS071(){
   ...
   /**
  * @checkChaineCommercialeOIS039 - Gets ITDS/FUDS/CFI1 from MITMAS
  * @params -
  * @returns -
  */
  void checkChaineCommercialeOIS039(){
    ...
    queryOCHCUS.readAll(containerOCHCUS,2,100, { DBContainer container ->
