...
* 1.0.1      161225  KMOTEAN     Updated method retrieveTOMU/Removed Filter AVTP/Updated output fields when listFinalRoutesNonRFO is empty/Updated output ITDS when item is unknown
* 1.0.2      150126  KMOTEAN     Added ORTP and CUOR validation/Adjusted function calculateNextDeliveryDate to take date according to timezone
*/
...
public class LstOrderSTD extends ExtendM3Transaction {
  ...
  public LstOrderSTD(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
  ...
  public void main() {
    ...
    if(!validateCONO()){
      ...
      mi.outData.put('ECHK', "header")
	    mi.outData.put('NEPR', "0.00")
      mi.outData.put('SACD', "0.00")
      mi.outData.put('SAPR', "0.00")
      mi.outData.put('LNAM', "0.00")
      mi.outData.put('VATC', "0.00")
      mi.outData.put('VTAM', "0.00")
      mi.outData.put('PBQA', "0")
      mi.outData.put('TOAM', "0.00")
      mi.outData.put('CRAM', "0.00")
      mi.outData.put('CONQ', "0")
      mi.write()
      return
    }
    ...
    if(!validateCUNO()){
      ...
      mi.outData.put('LNTX', "Code client non valide")
    ...
    if(stat610!="20"){
      ...
      mi.outData.put('ORQA', inORQA.intValue().toString())
      mi.outData.put('LNST', "R")
      mi.outData.put('LNTX', "Client non autorisé - statut est 12")
      mi.outData.put('CONM', divisionName)
      mi.outData.put('ECHK', "header")
      mi.outData.put('NEPR', "0.00")
      mi.outData.put('SACD', "0.00")
    ...
    if(creditLimitExceeded){
      ...
      mi.outData.put('ORQA', inORQA.intValue().toString())
      mi.outData.put('LNST', "R")
      mi.outData.put('LNTX', "Commande reçue payeur bloqué contacter le service comptabilité clients")
      mi.outData.put('CONM', divisionName)
      mi.outData.put('ECHK', "header")
      mi.outData.put('NEPR', "0.00")
      mi.outData.put('SACD', "0.00")
    ...
    if(!validateORTP()){
      ...
      mi.outData.put('ORQA', inORQA.intValue().toString())
      mi.outData.put('LNST', "R")
      mi.outData.put('LNTX', "Type de commande de vente ${inORTP} n'existe pas")
      mi.outData.put('CONM', divisionName)
      mi.outData.put('ECHK', "header")
      mi.outData.put('NEPR', "0.00")
      mi.outData.put('SACD', "0.00")
    ...
    if(retrievedCUOR=="*M" || retrievedCUOR=="*M1"){
      if(inCUOR==""){
        ...
        mi.outData.put('ORQA', inORQA.intValue().toString())
        mi.outData.put('LNST', "R")
        mi.outData.put('LNTX', "Numéro de commande du client est obligatoire")
        mi.outData.put('CONM', divisionName)
        mi.outData.put('ECHK', "header")
        mi.outData.put('NEPR', "0.00")
        mi.outData.put('SACD', "0.00")
    ...
    if(inORQA==0){
      ...
      mi.outData.put('PBQA', "0")
      mi.outData.put('WHLO', whlo610)
      getwarehouseName(whlo610)
    ...
    if(!flagClientNonRFO){
      ...
      if(listRoutes.isEmpty()){
        ...
        mi.outData.put('LNTX', "Error Routes")
        mi.outData.put('CONM', divisionName)
        mi.outData.put('PBQA', "0")
        mi.outData.put('WHLO', whlo610)
        getwarehouseName(whlo610)
      ...
      if(correctITNOFound){
        ...
        int count=0
        boolean allConqZero = listFinalRoutes.every { record -> record.CONQ == "0.00" }
        ...
        if(allConqZero){
          if(listFinalRoutes.isEmpty()){
            ...
            mi.outData.put('CACR', codeMarqueACR)
            mi.outData.put('CEAN',inCEAN)
            mi.outData.put('AV01',"0.00")
            mi.outData.put('CODZ',"")
            mi.outData.put('CODZ',record1.CODZ)
            mi.outData.put('COHZ',"")
            mi.outData.put('COHZ',record1.COHZ)
            mi.outData.put('LNTX', calculatedAVTX)
            mi.outData.put('LNST', calculatedAVST)
            mi.outData.put('FUDS', retrievedFUDS)
            mi.outData.put('ITDS', retrievedITDS)
            ...
            if(listPrices.size()>0){
              ...
              BigDecimal netp = new BigDecimal(listPrices[0].NETP.toString())
              BigDecimal conq = new BigDecimal(record1.CONQ)
              BigDecimal result = netp.multiply(conq).setScale(2, RoundingMode.HALF_UP)
              mi.outData.put("LNAM", result.toString().trim())
              mi.outData.put('NEPR',listPrices[0].NETP.toString())
              ...
              if(listDiscounts.size()==0){
                mi.outData.put('DIP1',"0.00")
                mi.outData.put('DIP2',"0.00")
              }
              else if(listDiscounts.size()==1){
                mi.outData.put('DIP1',listDiscounts[0])
                mi.outData.put('DIP2',"0.00")
              }
              else if(listDiscounts.size()>=2){
                mi.outData.put('DIP1',listDiscounts[0])
                mi.outData.put('DIP2',listDiscounts[1])
              }
              
              double vtam=((result.toString().trim().toDouble()*tva)/100)
              mi.outData.put('VTAM',  String.format("%.2f",vtam ))
              mi.outData.put('TOAM',  String.format("%.2f",vtam+result.toString().trim().toDouble() ))
            }   
      ...
      filteredList = listFinalRoutesNonRFO.groupBy { it.WHLO }.collect { whlo, records ->
          def withPbqa = records.findAll { (it.AV01 as BigDecimal) > 0 }
          withPbqa ? withPbqa.first() : records.first()
      }
      ...
      BigDecimal pbqa = (listFinalRoutesNonRFO.sum { 
      ...
      String calculatedAVST = ""
      if (!correctITNOFound) {
          calculatedAVTX = "Référence inconnue"
          calculatedAVST = "I"

      } else if (pbqa == null || pbqa.compareTo(BigDecimal.ZERO) == 0) {
          calculatedAVTX = "Indisponible"
          calculatedAVST = "I"

      } else if (pbqa.compareTo(inORQA) >= 0) {
          calculatedAVTX = "Disponible"
          calculatedAVST = "D"
      ...
      if (listFinalRoutesNonRFO.isEmpty()) {
      ...
      else if (calculatedAVTX.equals("Indisponible")) {
        Map<String, String> record1 = listFinalRoutesNonRFO.first()
        mi.outData.put('CONO', inCONO.toString())
        mi.outData.put('DIVI', inDIVI)
        mi.outData.put('CONM', divisionName)
        mi.outData.put('LNUM', inLNUM.toString())
        ...
        retrieveNonRFORouteInfoFromDROUTE()
        listFinalRoutesNonRFO.each { record1 ->

          // Exclude warehouses with no stock allocated
          if ((record1.AV01 as BigDecimal) != 0) {
            ...
            Map<String, String> match = listRoutesDetails.find { it.ROUT == record1.ROUT }
            String tx40Value = match ? match.TX40 : ""

            mi.outData.put('TX40',tx40Value)

            mi.outData.put('DPAN', record1.WHLO == whlo610 ? "false" : "true")
            ...
            if (listPrices.size() > 0) {
                ...
                BigDecimal netp = new BigDecimal(listPrices[0].NETP.toString())
                BigDecimal conq = new BigDecimal(record1.AV01)
                BigDecimal result = netp.multiply(conq).setScale(2, RoundingMode.HALF_UP)
                mi.outData.put("LNAM", result.toString().trim())
                ...
                double vtam = (result.doubleValue() * tva) / 100.0
                mi.outData.put('VTAM', String.format("%.2f", vtam))
                mi.outData.put('TOAM', String.format("%.2f", vtam + result.doubleValue()))
            }
            mi.write()
          }
        }
  ...
  * @returns -
  */
  public void retrieveCodeMarqueM3(String cono,String a230) {
    listCodeMarqueCugex=[]
  ...
  * @returns -
  */
  public void getCodeMarquePartenaire(String pk03) {
  ...
  */
  Boolean validateCUNO() {
    if (!inCUNO.isBlank()) {
      DBAction query = database.table("OCUSMA").index("00").selection("OKORTP","OKSPLM","OKWHLO","OKPADL","OKBCKO","OKSTAT","OKACHK","OKTXAP","OKVTCD","OKPYNO","OKMCOS").build()
      DBContainer container = query.getContainer()
  ...
  */
  void searchMITBALInfo(){
    ...
    queryMIBAL.readAll(containerMITBAL,1,1000, { DBContainer container ->
      ...
      Double quantity1=avalNum - alqtNum

      String stqt=container.get("MBSTQT").toString().trim()
      String reqt=container.get("MBREQT").toString().trim()
      
      Double stqtNum = Double.parseDouble(stqt)
      Double reqtNum = Double.parseDouble(reqt)

      Double quantity2=avalNum - alqtNum
      
      Double calculatedAV01=0

      if(quantity1<quantity2){
        calculatedAV01=quantity1
      }
      else{
        calculatedAV01=quantity2
      }
      ...
      listRoutesAllDetails.each { record1 ->
          if(record1.WHLO==container.get("MBWHLO").toString().trim() && record1.ITNO==container.get("MBITNO").toString().trim() ){
            ...
            record1.hasFicheArticleDepot=true
          }
      }
    })
  }
  ...
  */
  void retrieveFastestRoute() {
    ...
    boolean allAV01Zero = listRoutesAllDetails.every { it.AV01 == "0.00" || it.AV01 == "0"}
    if (allAV01Zero) {
    ...
    boolean allORQAZero = listRoutesAllDetails.every { it.ORQA == "0.00" || it.ORQA == "0"}
    if (allORQAZero) {
        listFinalRoutes = listRoutesAllDetails
        listRoutesAllDetails.each { item ->
          item.CONQ = "0.00"
    ...
    listRoutesAllDetails.each { entry -> 
        ...
        warehouseConsumed[warehouse] = (warehouseConsumed.get(warehouse, 0.00 as Double) + itemsToTake)
    ...
    if (listFinalRoutes.isEmpty()) {
        listFinalRoutes = listRoutesAllDetails
        listRoutesAllDetails.each { item -> item.CONQ = "0.00" }
  ...
  */
  void checkCreditLimit(String cuno) {
    ...
    /**
  * @description - Gets warehouse name
  * @params - WHLO
  * @returns - 
  */
  ...
  */
        ...
        if (responseOIS320MI != null) {
            if (responseOIS320MI.containsKey("error") && responseOIS320MI.error != null) {
                ...
                Map<String, String> filteredMap = [
                    ...
                    "AVTP": responseOIS320MI.AVTP?.toString()?.trim()
                ]
  ...
  /**
  * @description - Retrieve route info
  * @params -
  * @returns -
  */
  void retrieveNonRFORouteInfoFromDROUTE(){
    List<String> arrUniqueROUT = listFinalRoutesNonRFO.collect { it.ROUT }
    arrUniqueROUT=arrUniqueROUT.unique()
