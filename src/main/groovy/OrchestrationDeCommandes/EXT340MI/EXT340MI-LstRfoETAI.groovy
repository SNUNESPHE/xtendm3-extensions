...
public class LstRfoETAI extends ExtendM3Transaction {
  ...
  public LstRfoETAI(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
  ...
  public void main() {
    ...
    retrieveMITMASData()//Gets ITDS/FUDS/CFI1/ACHK from MITMAS
    ...
    ext320MIGetLine(inCONO.toString(),inCUNO, correctITNO)//getPrices
    ...
    listRoutesAllDetails.each { record1 ->
      record1.CFI1=codeMarqueM3
    }

    listRoutesAllDetails.each { record1 ->
      retievedTOMU=retrieveTOMU(inCONO.toString(),record1.WHLO,correctITNO)//retrieve TOMU from MITBAL
        record1.TOMU=retievedTOMU
        // Calculate real available value
        if(retievedTOMU!="" && record1.AV01.toString().trim()!=""){
          Double av01 = record1.AV01 as Double
          Double tomu = retievedTOMU as Double
          Double pbqa = 0
          ...
          record1.AV01=pbqa.toString()
        }
    }

    boolean EtaiRecordFound=false
    
    if (inOALT == 0) {
      Map<String, String>  record1 = listRoutesAllDetails.find { it.AV01 != "0.0" }
      if (record1) {
          EtaiRecordFound = true
          mi.outData.put('CONO', inCONO.toString())
          mi.outData.put('DIVI', inDIVI)
          mi.outData.put('CONM', divisionName)
          mi.outData.put('LNST', "ack")
          ...
          mi.outData.put('AVTX', record1.AVTX)
          mi.outData.put('AVTX', "known")
          mi.outData.put('CFI1', inCFI1)
          ...
          if (listPrices.size() > 0) {
              ...
              mi.outData.put('SPUN', listPrices[0].SPUN.toString())
          }
      ...
      // If no record with AV01 != "0.0", use the first record
      Map<String, String>  fallbackRecord = listRoutesAllDetails.first()
      EtaiRecordFound = true
      mi.outData.put('CONO', inCONO.toString())
      mi.outData.put('DIVI', inDIVI)
      mi.outData.put('CONM', divisionName)
      mi.outData.put('LNST', "ack")
      mi.outData.put('LNTX', "complete")
      mi.outData.put('CUNO', fallbackRecord.CUNO)
      mi.outData.put('WHNM', fallbackRecord.WHNM)
      mi.outData.put('WHLO', fallbackRecord.WHLO)
      mi.outData.put('ITNO', fallbackRecord.ITNO)
      mi.outData.put('FUDS', retrievedFUDS)
      mi.outData.put('POPN', inPOPN)
      mi.outData.put('AV01', fallbackRecord.AV01)
      mi.outData.put('CODZ', fallbackRecord.CODZ)
      mi.outData.put('COHZ', fallbackRecord.COHZ)
      mi.outData.put('AVTX', fallbackRecord.AVTX)
      mi.outData.put('AVTX', "known")
      mi.outData.put('CFI1', inCFI1)
      mi.outData.put('TOMU', fallbackRecord.TOMU)
  ...
  */
  void getTIME() {
   ...
   /**
  * @retrieveMITMASData - Gets ITDS/FUDS/CFI1 from MITMAS
  * @params -
  * @returns -
  */
  void retrieveMITMASData(){
    DBAction queryMITMAS = database.table("MITMAS").index("00").selection("MMFUDS","MMCFI1","MMACHK").build()
    DBContainer containerMITMAS = queryMITMAS.getContainer()
    containerMITMAS.set("MMCONO", inCONO)
    ...
    if(!queryMITMAS.read(containerMITMAS)){
      ...
      codeMarqueM3=containerMITMAS.get("MMCFI1").toString().trim()
  ...
  * @returns -
  */
  void ext320MIGetLine(String CONO,String CUNO, String ITNO) {
    Map<String, String> paramsExt320MIGetLine = ["CONO": "${CONO}".toString(),"CUNO": "${CUNO}".toString(), "ITNO": "${ITNO}".toString(), "ORQA": "1", "ORTP": "WAV", "EOUT": "1"]
            ...
            if (responseEXT320MIGetLine.containsKey("error") && responseEXT320MIGetLine.error != null) {
              return
                ...
                listPrices.add([
                  ...
                  SPUN:responseEXT320MIGetLine.SPUN.toString().trim()
                ])
            }
        }
    }
