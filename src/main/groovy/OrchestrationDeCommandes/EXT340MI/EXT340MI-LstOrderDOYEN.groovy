...
* 1.0.2      161225  KMOTEAN     Updated method retrieveTOMU
...
public class LstOrderDOYEN extends ExtendM3Transaction {
  ...
  private boolean compareMODL=false
  private String whnm610=""
  private List<String> arrConvertedMODL=[]
  private String pyno610=""
  private String codeMarqueACR=""

  
  public LstOrderDOYEN(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
  ...
  public void main() {
    ...
    arrConvertedMODL=[]
    if(inMODL=="D"){
      arrConvertedMODL.add("H+4")
      compareMODL=true
    }
    else if(inMODL=="E"){
      arrConvertedMODL.add("J+1")
      arrConvertedMODL.add("SAM")
      arrConvertedMODL.add("EXP")
      compareMODL=true
    }
    else if(inMODL=="N"){
      arrConvertedMODL.add("CPT")
      arrConvertedMODL.add("ENL")
      compareMODL=true
    }
    else{
      compareMODL=false
    ...
    listRoutesAllDetails.each { record ->
      record.isInputMatch = (record.WHLO == inWHLO && arrConvertedMODL.contains(record.MMDL))
    }
  ...
  */
  Boolean validateCUNO() {
    if (!inCUNO.isBlank()) {
      DBAction query = database.table("OCUSMA").index("00").selection("OKORTP","OKSPLM","OKWHLO","OKPADL","OKBCKO","OKSTAT","OKACHK","OKTXAP","OKVTCD","OKPYNO").build()
      DBContainer container = query.getContainer()
