...
public class LstRfoSTANDARD extends ExtendM3Transaction {
  ...
  public LstRfoSTANDARD(MIAPI mi, DatabaseAPI database, ProgramAPI program, MICallerAPI miCaller) {
    this.mi = mi
      ...
      if(allConqZero){
        if(listFinalRoutes.isEmpty()){
          ...
          if(listPrices.size()>0){
            ...
            mi.outData.put('DIP1',"0.0")
            mi.outData.put('DIP2',"0.0")
            mi.outData.put('DIP3',"")
            mi.outData.put('DIP4',"")
            mi.outData.put('DIP5',"")
            mi.outData.put('DIP6',"")
  ...
  */
  Boolean validateCUNO() {
    if (!inCUNO.isBlank()) {
      DBAction query = database.table("OCUSMA").index("00").selection("OKORTP","OKSPLM","OKWHLO","OKPADL","OKBCKO","OKSTAT","OKACHK","OKTXAP","OKVTCD").build()
      DBContainer container = query.getContainer()
  ...
  */
  String retrieveTVA(String vatCode){
    DBAction queryCVATPC = database.table("CVATPC").index("10").selection("CVVTP1").build()
